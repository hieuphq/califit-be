...
func testTagToManyTagMaps(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, tagMapDBTypes, false, tagMapColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, tagMapDBTypes, false, tagMapColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	tagMap, err := a.TagMaps().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := TagSlice{&a}
	if err = a.L.LoadTagMaps(ctx, tx, false, (*[]*Tag)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.TagMaps); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.TagMaps = nil
	if err = a.L.LoadTagMaps(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.TagMaps); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testTagToManyAddOpTagMaps(t *testing.T) {
	var err error
	...
	foreigners := []*TagMap{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, tagMapDBTypes, false, strmangle.SetComplement(tagMapPrimaryKeyColumns, tagMapColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*TagMap{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddTagMaps(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.TagMaps[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.TagMaps[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.TagMaps().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testTagToManySetOpTagMaps(t *testing.T) {
	var err error
	...
	err = a.SetTagMaps(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetTagMaps(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
...
func testTagToManyRemoveOpTagMaps(t *testing.T) {
	var err error
	...
	err = a.AddTagMaps(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveTagMaps(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
...
var (
	tagDBTypes = map[string]string{`CreatedAt`: `timestamp with time zone`, `ID`: `uuid`, `Name`: `character varying`, `UpdatedAt`: `timestamp with time zone`}
	_          = bytes.MinRead
)
