...
var (
	userRoleColumns               = []string{"id", "role_id", "user_id", "created_at", "updated_at"}
	userRoleColumnsWithoutDefault = []string{"id", "role_id", "user_id"}
...
func (o *UserRole) RemoveUser(ctx context.Context, exec boil.ContextExecutor, related *User) error {
	var err error
	...
	for i, ri := range related.R.UserRoles {
		if queries.Equal(o.UserID, ri.UserID) {
			continue
		}

		ln := len(related.R.UserRoles)
		if ln > 1 && i < ln-1 {
			related.R.UserRoles[i] = related.R.UserRoles[ln-1]
		}
		related.R.UserRoles = related.R.UserRoles[:ln-1]
		break
	}
	return nil
}
...
// Removes o from all passed in related items' relationships struct (Optional).
func (o *UserRole) RemoveRole(ctx context.Context, exec boil.ContextExecutor, related *Role) error {
	var err error
...
func FindUserRole(ctx context.Context, exec boil.ContextExecutor, iD string, selectCols ...string) (*UserRole, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"user_role\" where \"id\"=$1", sel,
	)
...
func (o *UserRole) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"user_role\" WHERE \"id\"=$1"
...
func (o UserRoleSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	if o == nil {
		return 0, errors.New("models: no UserRole slice provided for delete all")
	}
...
func UserRoleExists(ctx context.Context, exec boil.ContextExecutor, iD string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"user_role\" where \"id\"=$1 limit 1)"
