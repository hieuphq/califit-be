...
func testUserToManyUserSessions(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, userSessionDBTypes, false, userSessionColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, userSessionDBTypes, false, userSessionColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	userSession, err := a.UserSessions().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadUserSessions(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UserSessions); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.UserSessions = nil
	if err = a.L.LoadUserSessions(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UserSessions); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManySetOpUserRoles(t *testing.T) {
	var err error
...
func testUserToManyRemoveOpUserRoles(t *testing.T) {
	var err error
...
func testUserToManyAddOpUserSessions(t *testing.T) {
	var err error
	...
	var a User
	var b, c, d, e UserSession

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, userDBTypes, false, strmangle.SetComplement(userPrimaryKeyColumns, userColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*UserSession{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, userSessionDBTypes, false, strmangle.SetComplement(userSessionPrimaryKeyColumns, userSessionColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*UserSession{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddUserSessions(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.UserSessions[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.UserSessions[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.UserSessions().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testUserToManySetOpUserSessions(t *testing.T) {
	var err error
	...
	err = a.SetUserSessions(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetUserSessions(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if a.R.UserSessions[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.UserSessions[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testUserToManyRemoveOpUserSessions(t *testing.T) {
	var err error
	...
	err = a.AddUserSessions(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveUserSessions(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.UserSessions) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.UserSessions[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.UserSessions[0] != &e {
		t.Error("relationship to e should have been preserved")
	}
}

func testUserToOneImageUsingImage(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadImage(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToOneSetOpImageUsingImage(t *testing.T) {
	var err error
...
func testUserToOneRemoveOpImageUsingImage(t *testing.T) {
	var err error
...
var (
	userDBTypes = map[string]string{`CreatedAt`: `timestamp with time zone`, `Email`: `character varying`, `EncryptPassword`: `character varying`, `FBID`: `character varying`, `ID`: `uuid`, `Identity`: `character varying`, `ImageID`: `uuid`, `Name`: `character varying`, `UpdatedAt`: `timestamp with time zone`}
	_           = bytes.MinRead
)
