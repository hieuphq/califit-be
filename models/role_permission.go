...
type RolePermission struct {
	...
	PermissionID null.String `boil:"permission_id" json:"permission_id,omitempty" toml:"permission_id" yaml:"permission_id,omitempty"`
...
var (
	rolePermissionColumns               = []string{"id", "role_id", "permission_id", "created_at", "updated_at"}
	rolePermissionColumnsWithoutDefault = []string{"id", "role_id", "permission_id"}
...
func (o *RolePermission) Permission(mods ...qm.QueryMod) permissionQuery {
	...
	queries.SetFrom(query.Query, "\"permission\"")
...
func (rolePermissionL) LoadRole(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRolePermission interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`role`), qm.WhereIn(`id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
func (rolePermissionL) LoadPermission(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRolePermission interface{}, mods queries.Applicator) error {
	...
	Outer:
		for _, obj := range slice {
			...
			for _, a := range args {
				if queries.Equal(a, obj.PermissionID) {
					continue Outer
				}
			}
	...
	query := NewQuery(qm.From(`permission`), qm.WhereIn(`id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results of eager load for permission")
	}
	if err = results.Err(); err != nil {
...
func FindRolePermission(ctx context.Context, exec boil.ContextExecutor, iD string, selectCols ...string) (*RolePermission, error) {
...
func (o RolePermissionSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	if o == nil {
		return 0, errors.New("models: no RolePermission slice provided for delete all")
	}
...
func RolePermissionExists(ctx context.Context, exec boil.ContextExecutor, iD string) (bool, error) {
	var exists bool
