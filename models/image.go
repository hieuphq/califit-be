...
var (
	imageColumns               = []string{"id", "url", "source", "created_at", "updated_at"}
	imageColumnsWithoutDefault = []string{"id", "url", "source"}
...
// ImageMaps retrieves all the image_map's ImageMaps with an executor.
func (o *Image) ImageMaps(mods ...qm.QueryMod) imageMapQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"image_map\".\"image_id\"=?", o.ID),
	)

	query := ImageMaps(queryMods...)
	queries.SetFrom(query.Query, "\"image_map\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"image_map\".*"})
	}
...
func (o *Image) Trainers(mods ...qm.QueryMod) trainerQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"trainer\".\"image_id\"=?", o.ID),
	)
	...
	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"trainer\".*"})
	}
...
func (o *Image) Users(mods ...qm.QueryMod) userQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
...
// LoadImageMaps allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (imageL) LoadImageMaps(ctx context.Context, e boil.ContextExecutor, singular bool, maybeImage interface{}, mods queries.Applicator) error {
	var slice []*Image
	var object *Image
	...
	query := NewQuery(qm.From(`image_map`), qm.WhereIn(`image_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load image_map")
	}

	var resultSlice []*ImageMap
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice image_map")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on image_map")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for image_map")
	}
	...
	if singular {
		object.R.ImageMaps = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.ImageID) {
				local.R.ImageMaps = append(local.R.ImageMaps, foreign)
				if foreign.R == nil {
					foreign.R = &imageMapR{}
				}
...
func (imageL) LoadTrainers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeImage interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`trainer`), qm.WhereIn(`image_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on trainer")
	}
	if err = results.Err(); err != nil {
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (imageL) LoadUsers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeImage interface{}, mods queries.Applicator) error {
	var slice []*Image
	var object *Image
	...
	query := NewQuery(qm.From(`user`), qm.WhereIn(`image_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
// AddImageMaps adds the given related objects to the existing relationships
// of the image, optionally inserting them as new records.
// Appends related to o.R.ImageMaps.
// Sets related.R.Image appropriately.
func (o *Image) AddImageMaps(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*ImageMap) error {
	var err error
	...
	if o.R == nil {
		...
		o.R.ImageMaps = append(o.R.ImageMaps, related...)
	}
...
// SetImageMaps removes all previously related items of the
// image replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Image's ImageMaps accordingly.
// Replaces o.R.ImageMaps with related.
// Sets related.R.Image's ImageMaps accordingly.
func (o *Image) SetImageMaps(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*ImageMap) error {
	query := "update \"image_map\" set \"image_id\" = null where \"image_id\" = $1"
	...
	return o.AddImageMaps(ctx, exec, insert, related...)
}

// RemoveImageMaps relationships from objects passed in.
// Removes related items from R.ImageMaps (uses pointer comparison, removal does not keep order)
// Sets related.R.Image.
func (o *Image) RemoveImageMaps(ctx context.Context, exec boil.ContextExecutor, related ...*ImageMap) error {
	var err error
	for _, rel := range related {
		queries.SetScanner(&rel.ImageID, nil)
		if rel.R != nil {
	...
	for _, rel := range related {
		for i, ri := range o.R.ImageMaps {
			if rel != ri {
				continue
			}

			ln := len(o.R.ImageMaps)
			if ln > 1 && i < ln-1 {
				o.R.ImageMaps[i] = o.R.ImageMaps[ln-1]
			}
			o.R.ImageMaps = o.R.ImageMaps[:ln-1]
			break
		}
	}
...
// SetTrainers removes all previously related items of the
// image replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Image's Trainers accordingly.
// Replaces o.R.Trainers with related.
// Sets related.R.Image's Trainers accordingly.
func (o *Image) SetTrainers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Trainer) error {
	query := "update \"trainer\" set \"image_id\" = null where \"image_id\" = $1"
	values := []interface{}{o.ID}
	if boil.DebugMode {
		fmt.Fprintln(boil.DebugWriter, query)
		fmt.Fprintln(boil.DebugWriter, values)
	...
	return o.AddTrainers(ctx, exec, insert, related...)
}

// RemoveTrainers relationships from objects passed in.
// Removes related items from R.Trainers (uses pointer comparison, removal does not keep order)
// Sets related.R.Image.
func (o *Image) RemoveTrainers(ctx context.Context, exec boil.ContextExecutor, related ...*Trainer) error {
	var err error
	for _, rel := range related {
		queries.SetScanner(&rel.ImageID, nil)
		if rel.R != nil {
	...
	for _, rel := range related {
		for i, ri := range o.R.Trainers {
			...
			ln := len(o.R.Trainers)
			if ln > 1 && i < ln-1 {
				o.R.Trainers[i] = o.R.Trainers[ln-1]
			}
			o.R.Trainers = o.R.Trainers[:ln-1]
			break
		}
	}
...
// Sets related.R.Image appropriately.
func (o *Image) AddUsers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*User) error {
	var err error
	for _, rel := range related {
		if insert {
			queries.Assign(&rel.ImageID, o.ID)
...
// image replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Image's Users accordingly.
// Replaces o.R.Users with related.
// Sets related.R.Image's Users accordingly.
func (o *Image) SetUsers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*User) error {
	query := "update \"user\" set \"image_id\" = null where \"image_id\" = $1"
	values := []interface{}{o.ID}
	if boil.DebugMode {
		fmt.Fprintln(boil.DebugWriter, query)
		fmt.Fprintln(boil.DebugWriter, values)
...
// Sets related.R.Image.
func (o *Image) RemoveUsers(ctx context.Context, exec boil.ContextExecutor, related ...*User) error {
	var err error
	for _, rel := range related {
		queries.SetScanner(&rel.ImageID, nil)
		if rel.R != nil {
...
func (o *Image) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert image")
	}
