...
type Schedule struct {
	...
	CenterID  null.String `boil:"center_id" json:"center_id,omitempty" toml:"center_id" yaml:"center_id,omitempty"`
...
var (
	scheduleColumns               = []string{"id", "center_id", "start_at", "end_at", "created_at", "updated_at"}
	scheduleColumnsWithoutDefault = []string{"id", "center_id", "start_at", "end_at"}
	scheduleColumnsWithDefault    = []string{"created_at", "updated_at"}
...
// Center pointed to by the foreign key.
func (o *Schedule) Center(mods ...qm.QueryMod) centerQuery {
	queryMods := []qm.QueryMod{
		qm.Where("id=?", o.CenterID),
	}
...
func (o *Schedule) ScheduleDetails(mods ...qm.QueryMod) scheduleDetailQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"schedule_detail\".\"schedule_id\"=?", o.ID),
	)
...
// LoadCenter allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for an N-1 relationship.
func (scheduleL) LoadCenter(ctx context.Context, e boil.ContextExecutor, singular bool, maybeSchedule interface{}, mods queries.Applicator) error {
	var slice []*Schedule
	var object *Schedule
	...
	if singular {
		...
		if !queries.IsNil(object.CenterID) {
			args = append(args, object.CenterID)
		}
	...
	Outer:
		for _, obj := range slice {
			...
			for _, a := range args {
				if queries.Equal(a, obj.CenterID) {
					continue Outer
				}
			}

			if !queries.IsNil(obj.CenterID) {
				args = append(args, obj.CenterID)
			}

		}
	}

	query := NewQuery(qm.From(`center`), qm.WhereIn(`id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results of eager load for center")
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if queries.Equal(local.CenterID, foreign.ID) {
				local.R.Center = foreign
				if foreign.R == nil {
					foreign.R = &centerR{}
				}
...
func (scheduleL) LoadScheduleDetails(ctx context.Context, e boil.ContextExecutor, singular bool, maybeSchedule interface{}, mods queries.Applicator) error {
	var slice []*Schedule
	var object *Schedule
	...
	query := NewQuery(qm.From(`schedule_detail`), qm.WhereIn(`schedule_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
// SetCenter of the schedule to the related item.
// Sets o.R.Center to related.
// Adds o to related.R.Schedules.
func (o *Schedule) SetCenter(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Center) error {
	var err error
	if insert {
	...
	queries.Assign(&o.CenterID, related.ID)
	if o.R == nil {
		o.R = &scheduleR{
			Center: related,
		}
...
// RemoveCenter relationship.
// Sets o.R.Center to nil.
// Removes o from all passed in related items' relationships struct (Optional).
func (o *Schedule) RemoveCenter(ctx context.Context, exec boil.ContextExecutor, related *Center) error {
	var err error

	queries.SetScanner(&o.CenterID, nil)
	if _, err = o.Update(ctx, exec, boil.Whitelist("center_id")); err != nil {
		return errors.Wrap(err, "failed to update local table")
	}
	...
	for i, ri := range related.R.Schedules {
		if queries.Equal(o.CenterID, ri.CenterID) {
			continue
		}

		ln := len(related.R.Schedules)
		if ln > 1 && i < ln-1 {
			related.R.Schedules[i] = related.R.Schedules[ln-1]
		}
		related.R.Schedules = related.R.Schedules[:ln-1]
		break
	}
	return nil
}
...
func (o *Schedule) AddScheduleDetails(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*ScheduleDetail) error {
	var err error
...
// Sets o.R.Schedule's ScheduleDetails accordingly.
// Replaces o.R.ScheduleDetails with related.
// Sets related.R.Schedule's ScheduleDetails accordingly.
func (o *Schedule) SetScheduleDetails(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*ScheduleDetail) error {
	query := "update \"schedule_detail\" set \"schedule_id\" = null where \"schedule_id\" = $1"
...
func (o *Schedule) RemoveScheduleDetails(ctx context.Context, exec boil.ContextExecutor, related ...*ScheduleDetail) error {
	var err error
	for _, rel := range related {
		queries.SetScanner(&rel.ScheduleID, nil)
		if rel.R != nil {
...
func (o ScheduleSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	if o == nil {
		return 0, errors.New("models: no Schedule slice provided for delete all")
	}
