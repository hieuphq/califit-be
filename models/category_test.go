...
func testCategoryToManyClasses(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, classDBTypes, false, classColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, classDBTypes, false, classColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	class, err := a.Classes().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = a.L.LoadClasses(ctx, tx, false, (*[]*Category)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	...
	a.R.Classes = nil
	if err = a.L.LoadClasses(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Classes); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testCategoryToManyAddOpClasses(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		...
		count, err := a.Classes().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testCategoryToManySetOpClasses(t *testing.T) {
	var err error
	...
	err = a.SetClasses(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetClasses(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
...
func testCategoryToManyRemoveOpClasses(t *testing.T) {
	var err error
	...
	err = a.AddClasses(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveClasses(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
...
var (
	categoryDBTypes = map[string]string{`CreatedAt`: `timestamp with time zone`, `ID`: `uuid`, `Name`: `character varying`, `UpdatedAt`: `timestamp with time zone`}
	_               = bytes.MinRead
)
