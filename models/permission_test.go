...
var (
	permissionDBTypes = map[string]string{`Action`: `character varying`, `CreatedAt`: `timestamp with time zone`, `ID`: `uuid`, `Model`: `character varying`, `Name`: `character varying`, `UpdatedAt`: `timestamp with time zone`}
	_                 = bytes.MinRead
)
...
func testPermissionsUpdate(t *testing.T) {
	...
	if len(permissionColumns) == len(permissionPrimaryKeyColumns) {
...
func testPermissionsSliceUpdateAll(t *testing.T) {
	...
	var fields []string
	if strmangle.StringSliceMatch(permissionColumns, permissionPrimaryKeyColumns) {
